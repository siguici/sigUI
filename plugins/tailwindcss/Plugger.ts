...
export class Plugger implements PluggerContract {
    static DEFAULT_OPTIONS: PluggerOptions = {
        linkClass: "link",
        entryClass: "entry",
        buttonClass: "button",
    };
    ...
    constructor(
        ...
        readonly options: PluggerOptions = Plugger.DEFAULT_OPTIONS,
    ) {
        const { config } = api;
    ...
    public plug(): this {
        return this.plugColors().plugLinks().plugEdge();
    }

    public plugColors(): this {
        for (const name in colors) {
            this.addColor(name, this.getDetailsColorOf(name));
        }
        return this;
    }

    public plugLinks(): this {
        const { linkClass } = this.options;
    ...
    public plugEdge(): this {
        const { spacing, borderWidth, borderRadius } = defaultTheme;
        const { entryClass, buttonClass } = this.options;
    ...
    public addColor(name: string, variants: ColorVariants): this {
    ...
    public matchColorValues(name: string, values: ColorValues): this {
    ...
    public addColorValue(name: string, value: ColorValue): this {
